# Synopsys dc_shell Backend

Synopsys Design Compiler shell (dc_shell) — the Tcl-based interactive shell
for RTL synthesis.  Maps RTL (Verilog/VHDL) to a gate-level netlist
...
- **Utility:** `current_design`, `list_libs`, `get_cells`, `get_nets`,
  ...
  `change_names`, `shell_is_in_exploration_mode`

## Key Concepts

- **target_library** — `.db` files for the standard-cell library to map to
- **link_library** — all libraries loaded at link time (`"*"` includes the
  in-memory design); typically `"* $target_library"`
- **synthetic_library** — DesignWare `.sldb` files for datapath components
- **symbol_library** — generic `.sldb` for schematic symbols
- **search_path** — directories searched for library and source files
- **define_design_lib WORK** — sets up the working design library
- `.ddc` — Synopsys internal binary format (full design state)
- `.db` — compiled Liberty library binary format
- `.sldb` — synthetic (DesignWare) library format
...
- `compile_ultra` is the primary synthesis command (higher QoR than `compile`).
  Use `-gate_clock` to insert clock gating and `-scan` for test-ready.
- `change_names -rules verilog -hierarchy` is run before writing netlists
  to produce synthesis-friendly names.
- `set_svf -on` enables SVF writing for formal equivalence with Formality.
